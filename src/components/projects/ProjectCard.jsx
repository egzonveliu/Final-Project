    ...
    <div className="card group relative flex flex-col gap-4 hover:border-accent/20">
      ...
      <div className="flex items-center justify-between mt-auto pt-2 border-t border-gray-200 dark:border-white/[0.05]">
        ...
        <div className="flex items-center gap-2">
          {isAdmin && (
            <>
              <button onClick={() => onEdit(project)} className="text-gray-400 dark:text-white/30 hover:text-accent transition-colors" aria-label="Edit">
                <Pencil size={14} />
              </button>
              <button onClick={() => onDelete(project.id)} className="text-gray-400 dark:text-white/30 hover:text-red-400 transition-colors" aria-label="Delete">
                <Trash2 size={14} />
              </button>
            </>
          )}
          <a href={project.link} target="_blank" rel="noreferrer" className="text-gray-400 dark:text-white/30 hover:text-accent transition-colors" aria-label="View project">
