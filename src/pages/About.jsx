...
function AboutHero() {
  return (
    <section className="max-w-6xl mx-auto px-6 pt-16 pb-10">
      ...
      <p className="max-w-2xl text-base text-white/55 leading-relaxed">
  ...
  return (
    <section className="border-t border-white/[0.06] py-16">
      <div className="max-w-6xl mx-auto px-6">
        <h2 className="text-2xl font-bold text-white/90 mb-10">Technical Skills</h2>
                  ...
                  {items.map(skill => (
                    <div key={skill.id}>
                      <div className="flex justify-between mb-1">
                        <span className="text-sm text-white/75">{skill.name}</span>
          ...
          <div className="h-72 md:h-auto">
            <ResponsiveContainer width="100%" height={280}>
              <RadarChart data={radarData}>
                ...
                <PolarAngleAxis dataKey="subject" tick={{ fill: 'rgba(255,255,255,0.35)', fontSize: 11, fontFamily: 'DM Mono' }} />
                <Radar name="Skills" dataKey="A" stroke="#7cffa4" fill="#7cffa4" fillOpacity={0.12} strokeWidth={1.5} />
                <Tooltip
    ...
    <section className="border-t border-white/[0.06] py-16">
      <div className="max-w-6xl mx-auto px-6">
        <div className="grid md:grid-cols-2 gap-12">
          <div>
            <h2 className="text-2xl font-bold text-white/90 mb-6">Soft Skills</h2>
            <div className="flex flex-wrap gap-2">
              {soft.map(s => (
                <span key={s} className="text-sm px-3 py-2 bg-white/[0.03] border border-white/[0.07] rounded text-white/55">
                  {s}
                </span>
              ))}
            </div>
          ...
          <div>
            <h2 className="text-2xl font-bold text-white/90 mb-6">Languages</h2>
            {[{ lang: 'Albanian', level: 'Native', pct: 100 }, { lang: 'English', level: 'Good', pct: 70 }].map(l => (
              <div key={l.lang} className="mb-5">
                <div className="flex justify-between mb-1">
                  <span className="text-sm text-white/75">{l.lang}</span>
